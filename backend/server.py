...
async def register(input: RegisterRequest):
    ...
    member = Member(name=input.name, email=input.email, phone=input.phone, role=role, bio=input.bio)
...
async def login(input: LoginRequest):
    ...
    if not verify_password(input.password, member_doc.get("password_hash", "")):
        raise HTTPException(status_code=401, detail="Invalid email or password")
...
async def add_comment(verse_id: str, comment: CommentCreate):
    comment_data = {"id": str(uuid.uuid4()), "member_id": comment.member_id, "member_name": comment.member_name, "text": comment.text, "created_at": datetime.utcnow().isoformat()}
...
async def get_messages(member_id: str, conversation_with: Optional[str] = None, group_id: Optional[str] = None):
    if group_id:
        ...
        query = {"$or": [{"sender_id": member_id, "recipient_id": conversation_with}, {"sender_id": conversation_with, "recipient_id": member_id}]}
    else:
...
async def get_conversations(member_id: str):
    messages = await db.messages.find({"$or": [{"sender_id": member_id}, {"recipient_id": member_id}], "group_id": None}).to_list(10000)
    conversations = {}
    for msg in messages:
        partner_id = msg["recipient_id"] if msg["sender_id"] == member_id else msg["sender_id"]
        if partner_id and (partner_id not in conversations or msg["created_at"] > conversations[partner_id]["created_at"]):
