...
class StudySession(BaseModel):
    ...
    group_id: Optional[str] = None  # None means main group
    host_id: str
...
class PrayerRequest(BaseModel):
    ...
    group_id: Optional[str] = None
    praying_members: List[str] = []  # member IDs
    created_at: datetime = Field(default_factory=datetime.utcnow)
...
class Announcement(BaseModel):
    ...
    author_id: str
    author_name: str
    group_id: Optional[str] = None  # None means for all
...
@api_router.get("/verses/{verse_id}", response_model=VerseDiscussion)
async def get_verse_discussion(verse_id: str):
    verse = await db.verses.find_one({"id": verse_id})
    if not verse:
        raise HTTPException(status_code=404, detail="Verse discussion not found")
    return VerseDiscussion(**verse)
...
async def delete_verse_discussion(verse_id: str):
...
@api_router.get("/prayers/{prayer_id}", response_model=PrayerRequest)
async def get_prayer_request(prayer_id: str):
...
async def delete_prayer_request(prayer_id: str):
...
async def toggle_pin_announcement(announcement_id: str, pin: bool = True):
